...
function slideControl(root, options) {
	...
	var sIsRandom = options.isRandom

	function init() {
		if (sIsRandom) {
			var result = Math.floor(Math.random() * oSlideScrollContent_length);
			for (var i = 0; i < result; i++) {
				jQuery(oSlideScrollContents.obj[0]).append(jQuery(oSlideScrollContent.obj).eq(i).clone());
			}
			for (var i = 0; i < result; i++) {
				jQuery(oSlideScrollContent.obj).eq(i).remove();
			}
		}
	...
	function setEvent() {
		...
		jQuery(oSlideScrollContents.obj[0]).children("li").each(function (index) {
